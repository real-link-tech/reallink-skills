	...
	public class UnrealInsightsReader
	{
		...
		public Dictionary<ushort, MemoryStream> Demux(BinaryReader Reader)
		{
			...
			while (Reader.BaseStream.Position < Reader.BaseStream.Length)
			{
				...
				MemoryStream ThreadStream;
				if (!ThreadStreams.TryGetValue(Packet.GetThreadId(), out ThreadStream))
				{
				...
				ThreadStream.Write(PacketData, 0, PacketData.Length);
		...
		public Dictionary<ushort, List<ITraceEvent>> GetEventsPerUid()
		{
			...
			foreach (KeyValuePair<ushort, List<ITraceEvent>> ThreadEntry in EventsPerThread)
			{
				List<ITraceEvent> ThreadEvents = ThreadEntry.Value;
				foreach (ITraceEvent Event in ThreadEvents)
				{
					...
					if (!EventsPerUid.TryGetValue(Uid, out Events))
					{
		...
		{
			foreach (KeyValuePair<ushort, List<ITraceEvent>> Entry in GetEventsPerUid())
			{
				ushort Uid = Entry.Key;
				List<ITraceEvent> Events = Entry.Value;
